...
public class SimpleAsyncTask extends AsyncTask<Void,Integer, String> {
    ...
    private WeakReference<TextView> mTextTest;
    // Constructor that provides a reference to the TextView from the MainActivity
    SimpleAsyncTask(TextView tv, TextView tv2, ProgressBar tv3) {
        mTextView = new WeakReference<>(tv);
        mTextTest = new WeakReference<>(tv2);
        mprogressBar = new WeakReference<>(tv3);
    }
    ...
    protected String doInBackground(Void... voids) {
        ...
        for (int i = 0; i < s; i += 10){
            try {
                Thread.sleep(10);
                publishProgress((s*100-1000)/s,i*100/s);
...
//        }
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        mTextTest.get().setText(String.valueOf(values[1]));
        mprogressBar.get().setMax(values[0]);
